...
import argparse
triple_format = r'<(.*?)>'
logging.basicConfig(filename='./triple_compgcn/session_2/chatglm_train_2_rgcn_after_vae_for_session3.txt',
...
def _replace_module(parent_module, child_name, new_module, old_module):
    setattr(parent_module, child_name, new_module)# 改进model中
...
class Agent(nn.Module):
    def __init__(self,
        ...
        self.perosna_proj = nn.Sequential(
            ...
            nn.Linear(4096 * int(proj_mid), 4096 * proj_token_num), # self.chatglm_model.config.hidden_size
        ).to(self.device)
        ...
        self.query_vec_train = np.load("./vector_library/session_2/DATA/query_driven_train.npy")
        self.node_train = np.load("./vector_library/session_2/triple_from_doubao/node_train.npy")
        self.query_vec_test = np.load("./vector_library/session_2/DATA/query_driven_test.npy")
        self.node_test = np.load("./vector_library/session_2/triple_from_doubao/node_test.npy")
        with open("./vector_library/session_2/DATA/query2id_train.json") as f:
            self.query2id_train = json.load(f)
        with open("./vector_library/session_2/triple_from_doubao/node2id_train.json") as f:
            self.node2id_train = json.load(f)
        with open("./vector_library/session_2/DATA/query2id_test.json") as f:
            self.query2id_test = json.load(f)
        with open("./vector_library/session_2/triple_from_doubao/node2id_test.json") as f:
    ...
    def forward(self, data):
        ...
        input_ids, loss_masks = [
            self.model_tokenizer.get_command('[gMASK]'),
            self.model_tokenizer.get_command('sop'),
        ], [False, False]

        new_input_ids = self.model_tokenizer.build_single_message(
                    "system", '', sys_prompt
                )
        new_loss_masks = [False] * len(new_input_ids)
        ...
        new_loss_masks = [True] * len(new_input_ids)
        input_ids += new_input_ids
        loss_masks += new_loss_masks
        input_ids.append(self.model_tokenizer.eos_token_id)
        ...
        batched_input_ids = torch.LongTensor(batched_input_ids).to(self.model.device)
        batched_labels = torch.LongTensor(batched_labels).to(self.model.device)

        user_embedding = vae_z
        inputs_embeds = torch.cat([chatbot_embedding_.unsqueeze(0).unsqueeze(0).to(self.model.device), self.model.transformer.embedding(batched_input_ids)], dim = 0)
        batched_labels = torch.cat([torch.ones([1, 1],dtype=torch.long).to(self.model.device).fill_(-100), batched_labels], dim = 1)# batch, virtual_nu
        outputs = self.model(
    ...
    def generate(self, data):
        chatbot_embedding = self.persona_encoder(data, query=data['query'])
        ...
        output_prompt, output_ids = (
                        new_input_ids[:1],
                        new_input_ids[1:],)
        output_ids.append(self.model_tokenizer.eos_token_id)
        batched_input_ids.append(
                        input_ids[:max_input_length] + output_prompt[:1]
                    )
        batched_input_ids = torch.LongTensor(batched_input_ids).to(self.model.device)
        
        inputs_embeds = torch.cat([chatbot_embedding.unsqueeze(0).unsqueeze(0).to(self.model.device), self.model.transformer.embedding(batched_input_ids)], dim = 0)
        attention_mask = torch.ones(inputs_embeds.shape[1], inputs_embeds.shape[0])
        user_embedding = vae_z
        ...
        with torch.no_grad():
            if self.mode == 'train':
                ...
                if query not in self.query2id_test.keys():
                    ...
                    query_outputs = self.model.transformer(**query, output_hidden_states = True)
            ...
            for node in chatbot_node_dict:
                if self.mode == 'train':
                    ...
                    if node not in self.node2id_test.keys():
                        ...
                        #embedding = np.mean(first_hidden_states + last_hidden_states, axis=0)
                    else:
